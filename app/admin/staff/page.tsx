...
import { UserCheck, Search, RefreshCw, Phone, Mail, Loader2, Briefcase } from "lucide-react";
...
export default function AdminStaffPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // API returns { staff: [{ id, name, image, staffProfile: { designation, specializations, isAvailable, bio } }] }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(member => {
            ...
            const active  = member.isActive !== false; // default true if not set
            return (
              <div key={member.id} className="bg-white rounded-sm border border-cream-darker/50 p-5 hover:shadow-md transition-all">

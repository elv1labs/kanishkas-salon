  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Pending",    value: counts.PENDING,    icon: <Clock size={18} className="text-amber-500" /> },
          { label: "Processing", value: counts.PROCESSING, icon: <Package size={18} className="text-blue-500" /> },
          { label: "Shipped",    value: counts.SHIPPED,    icon: <Truck size={18} className="text-purple-500" /> },
          { label: "Delivered",  value: counts.DELIVERED,  icon: <CheckCircle size={18} className="text-green-500" /> },
        ].map(s => (
          <div key={s.label} className="bg-white rounded-sm border border-cream-darker/50 p-4 text-center">
            <div className="flex justify-center mb-1">{s.icon}</div>
            <p className="font-display text-xl font-bold text-espresso">{loading ? "—" : s.value}</p>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {order.status === "PROCESSING" && (
                              <button
                                ...
                                disabled={updating === order.id}
                                className="text-[10px] ml-1 px-2 py-1 text-red-500 hover:bg-red-50 rounded transition-colors">
                                Cancel
                              </button>
                            )}
                          </div>

...
```bash
# The upload pipeline uses sharp — no external service needed
# POST /api/upload (multipart/form-data, field name: 'file', optional: 'folder')
# Returns: { imageUrl, thumbnailUrl, width, height, sizeBytes }
# Images are saved to: public/uploads/<folder>/ (full WebP) and public/uploads/<folder>/thumbs/ (thumbnail WebP)
```

> Note: In Docker production, uploads are persisted via the `uploads_data` named volume.
> Run `docker compose down` NOT `docker compose down -v` to avoid wiping uploaded images.
...
# DATABASE_URL, NEXTAUTH_SECRET (use: openssl rand -base64 32), NEXTAUTH_URL, NEXT_PUBLIC_APP_URL

# ✅ OPTIONAL but recommended env vars:
# GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET (Google OAuth)
# RESEND_API_KEY (email notifications)
# TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE (SMS/WhatsApp)
# NEXT_PUBLIC_GOOGLE_MAPS_API_KEY (contact page map)

# ⚠️ DO NOT use docker compose down -v in production — this destroys the uploads_data volume
```
...
### Image Handling
- Gallery and product images are stored locally on VPS under `public/uploads/`.
- The `sharp` pipeline in `lib/storage.ts` handles all processing server-side — no CDN or external service needed.
- Always use native `<img>` tags for gallery images (not `next/image`) to avoid domain whitelist issues.
- The upload endpoint returns both `imageUrl` (full 1600px WebP) and `thumbnailUrl` (400x400 WebP) — always save both to the DB.

```typescript
// Correct pattern for image display in gallery
// Thumbnail in grid, full image in lightbox
<img src={item.thumbnailUrl || item.imageUrl} alt={item.altText} loading="lazy" />
// eslint-disable-next-line @next/next/no-img-element
<img src={item.imageUrl} alt={item.altText} className="max-h-[80vh]" />
```
...
> Updated: April 3, 2026. Ordered by business impact.
...
- [x] **Cart State** — CartContext replacing localStorage; Cart icon in header
- [x] **Offline Payment Tracking** — `/api/appointments/mark-paid` endpoint
- [x] **PWA QR Install Cards** — Self-contained HTML file for in-salon distribution
- [x] **HTTPS Setup** — Self-signed SSL + ngrok tunnel for trusted PWA install

### 🔴 Sprint 4 — Infrastructure & Notifications (NEXT)

- [ ] **Domain Purchase** — Buy `kanishkassalon.com` (₹800–₹1,200/yr)
- [ ] **DNS Configuration** — Point A record → `168.231.121.107`
- [ ] **Let's Encrypt SSL** — `certbot --nginx -d kanishkassalon.com` (free)
- [ ] **Env Var Update** — `NEXTAUTH_URL` and `NEXT_PUBLIC_APP_URL` → domain
- [ ] **QR Code Update** — Update `public/docs/install-qr.html` with live domain URL
- [ ] **Remove ngrok dependency** — Once domain + SSL is live
- [ ] **Activate Email** — `npm install resend`, set `RESEND_API_KEY`, wire `sendBookingNotification()` into appointment/order routes
- [ ] **Activate SMS** — `npm install twilio`, set `TWILIO_*` vars
- [ ] **Google OAuth** — Set `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`
- [ ] **Schema Cleanup** — Remove orphaned Stripe/Razorpay fields from `Payment` model, delete empty `/api/webhooks/stripe/` directory
...
- [ ] **Gift Voucher UI** — Purchase and redemption flow (page exists, needs full wiring)
- [ ] **Referral Dashboard** — Referral tracking and rewards UI

### 🔵 Future Enhancements

- [ ] **Android APK** — Bubblewrap TWA (after domain goes live)
- [ ] **Push Notifications** — PWA push via Firebase Cloud Messaging
- [ ] **Google Calendar Sync** — Sync appointments to staff Google Calendars
- [ ] **Razorpay Integration** — Indian UPI payment gateway as Stripe alternative

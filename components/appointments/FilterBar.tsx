...
const STATUS_OPTIONS  = ["PENDING","CONFIRMED","IN_PROGRESS","COMPLETED","CANCELLED","NO_SHOW"] as const;
...
export default function AppointmentsFilterBar({
  total, loading, services, staff, onFilterChange,
}: Props) {
  ...
  const [filters, setFilters] = useState<FilterState>(() => ({
    ...
    staffIds:       searchParams.get("staffId")   ? [searchParams.get("staffId")!]   : [],
  ...
  const pushFilters = useCallback((f: FilterState) => {
    ...
    if (f.serviceIds[0]) params.set("serviceId",     f.serviceIds[0]);
    if (f.staffIds[0])   params.set("staffId",       f.staffIds[0]);
  ...
  const toggleMulti = (
    key: "serviceIds" | "staffIds" | "statuses",
    value: string
  ) => {
  ...
  filters.serviceIds.forEach(id => {
    ...
    if (s) chips.push({ label: s.name, onRemove: () => toggleMulti("serviceIds", id) });
  });
  filters.staffIds.forEach(id => {
    const s = staff.find(x => x.id === id);
    if (s) chips.push({ label: s.name, onRemove: () => toggleMulti("staffIds", id) });
  });
  ...
  const FilterBarContent = () => (
    <>
      {/* Row 1: search + date range + date presets */}
      ...
      <div className="flex flex-wrap items-center gap-3 pt-1">
        <MultiSelect
          ...
          onToggle={id => toggleMulti("serviceIds", id)}
          display={s => s.name}
        />
        <MultiSelect
          label="Staff"
          ...
          onToggle={id => toggleMulti("staffIds", id)}
          display={s => s.name}
        />
        <MultiSelect
          label="Status"
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <span className="text-[10px] bg-gold/20 text-gold px-1.5 py-0.5 rounded-full font-bold">
              {activeFilterCount}
            </span>
          )}
